...
from googletrans import urls, utilsN
from googletrans.gtokenN import TokenAcquirer
from googletrans.constantsN import (
    DEFAULT_CLIENT_SERVICE_URLS,
    DEFAULT_FALLBACK_SERVICE_URLS,
    DEFAULT_USER_AGENT, LANGCODES, LANGUAGES, SPECIAL_CASES,
    DEFAULT_RAISE_EXCEPTION, DUMMY_DATA
)
from googletrans.modelsN import Translated, Detected, TranslatedPart
...
class WordTranslator:
    ...
    def _translate_legacy(self, text, dest, src, override):
        ...
        params = utilsN.build_params(client=self.client_type, query=text, src=src, dest=dest,
                                    token=token, override=override)

        url = urlsN.TRANSLATE.format(host=self._pick_service_url())
        r = self.client.get(url, params=params)

        if r.status_code == 200:
            data = utilsN.format_json(r.text)
            return data, r
