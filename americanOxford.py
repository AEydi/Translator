...
class Word(object):
    ...
    am_pronounce_audio_selector = '[geo=n_am] [data-src-mp3]'
    # senses_multiple <ol class="sn-gs"
    # sense <li class="sn-g"
    # examples <span class="x-gs"

    definition_body_selector = '.sn-gs'
    namespaces_selector = '.sn-gs > .shcut-g'
    examples_selector = '.sn-gs .sn-g > .x-gs .x'
    definitions_selector = '.sn-gs .sn-g > .def'

    extra_examples_selector = '.res-g [title="Extra x-gs"] .x-gs .x'
    ...
    def get_url(cls, word):
        ...
        baseurl = 'https://www.oxfordlearnersdictionaries.com/us/definition/american_english/'
    ...
    def get(cls, word):
        ...
        page_html = req.get(cls.get_url(word), timeout=10, headers={'User-agent': 'HiOxford'})
    ...
    def ids(cls):
        ...
        if cls.soup_data is None:
            return None
        idd = [cls.soup_data.select(cls.entry_selector)[0].attrs['id']]
        if '_1' in idd[0]:
            idd = []
            items = cls.soup_data.select('.nearby a')
            for item in items:
                link = item['href']
                i = re.search(r'' + cls.word + '_\d', link)
                if i:
                    idd.append(i.group(0))
        return idd
    ...
    def pronunciations(cls):
        ...
        america = {'ipa': None, 'url': None}

        try:
            america_pron_tag = cls.soup_data.select(cls.am_pronounce_selector)[0]

            america['ipa'] = america_pron_tag.text[6:-2]
        except IndexError:
            pass

        try:
            america['url'] = cls.soup_data.select(cls.am_pronounce_audio_selector)[0].attrs['data-src-mp3']
        except IndexError:
            pass
    ...
    def _parse_definition(cls, parent_tag):
        ...
        try:  # label: (old-fashioned), (informal), (saying)...
            definition['label'] = parent_tag.select('.label-g')[0].text
        except IndexError:
            pass
        ...
        definition['examples'] = [example_tag.text
                                  for example_tag in parent_tag.select('.x-gs .x')]

        definition['extra_example'] = [
            example_tag.text
            for example_tag in parent_tag.select('[unbox=extra_examples] .x-gs .unx')
        ]
    ...
    def definition_full(cls):
        ...
        for namespace_tag in namespace_tags:
            try:
                namespace = namespace_tag.select('span.shcut')[0].text
            except IndexError:
        ...
        if len(info) == 0:
            ...
            definitions = []
            definition_full_tags = def_body_tags[0].select('.sn-g')
    ...
    def idioms(cls):
        ...
        for idiom_tag in idiom_tags:
            ...
            try:  # label: (old-fashioned), (informal), (saying)...
                global_definition['label'] = idiom_tag.select('.label-g')[0].text
            except IndexError:
                pass
            ...
            for definition_tag in idiom_tag.select('.sn-g'):
                definition = {}
